...
async function find() { // EXERCISE A
  /*
    ...
    ANSWER: Inner join removes the scheme with no steps. 
...
async function add(scheme) { // EXERCISE D
  ...
  */
  ...
  let ids = await db('schemes').insert(scheme);
  console.log('inserted ids: ', ids);

  const scheme_id = ids[0];
  console.log(scheme_id);

  let result = await findById(scheme_id)
  console.log('found scheme: ', result);
...
async function addStep(scheme_id, step) { // EXERCISE E
  ...
  */
  ...
  //console.log(newStep, parseInt(step.step_number))
  await db('steps')
    .insert(newStep)
